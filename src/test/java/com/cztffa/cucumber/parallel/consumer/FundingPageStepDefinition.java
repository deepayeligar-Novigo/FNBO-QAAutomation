...
public class FundingPageStepDefinition {
	...
	@Then("^: I should see the account is required validation error\\.$")
	public void iShouldSeeTheAccountIsRequiredValidationError() throws Throwable {
	...
	public void iSelectTheOption(String account) throws Throwable {
		Thread.sleep(8000);
		browserActions.clickApply(seleniumdriver.getWebDriver(), reviewPage.getFundingPageModel().sourceOfFundDrApply);
		if (account.equalsIgnoreCase("ExternalAccount")) {
			browserActions.clickApply(seleniumdriver.getWebDriver(), reviewPage.getFundingPageModel().sourceOfFundOpApply);
		}
	}

	@Then("^: I should see the plaid validation error\\.$")
	public void iShouldSeeThePlaidValidationError() throws Throwable {
		assertTrue(reviewPage.getFundingPageModel().floaterMsgValidation.isDisplayed());
	}
}
