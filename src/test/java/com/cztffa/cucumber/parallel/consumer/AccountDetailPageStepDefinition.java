...
public class AccountDetailPageStepDefinition {
	...
	@And("^: I should see the account details page$")
	public void iShouldSeeTheAccountDetailsPage() throws Throwable {
		assertTrue(seleniumdriver.getWebDriver().getPageSource().contains("Y-12 Federal Credit Union"));
	}
	...
	@Then("^: I click on the account details next button for multiApplicant$")
	public void iClickOnTheAccountDetailsNextButtonForMultiApplicant() throws Throwable {
		JavascriptExecutor j = (JavascriptExecutor) seleniumdriver.getWebDriver();
		j.executeScript("window.scrollBy(1900, 2400)");
		Thread.sleep(3000);
		browserActions.clickButton(seleniumdriver, reviewPage.getAccountDetailsPageModel().accountDetailsNextButton);
	}

	@Then("^: I provide everyday accountdetails$")
	public void iProvideEverydayAccountdetails(Map<String, String> table) throws Throwable {
		JSONObject jsonObject = new JSONObject(table);
		ObjectMapper objectMapper = new ObjectMapper();
		AccountDetails accountDetails = objectMapper.readValue(jsonObject.toString(), AccountDetails.class);
		reviewPage.everyDayChecking(accountDetails);
	}
